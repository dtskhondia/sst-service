...
public class ProviderGroupController {
    ...
    @PostMapping()
    ResponseEntity<ProviderGroupDto> create(
        @Parameter(description = "Provider Group to Be Created")
        @RequestBody @Valid ProviderGroupDto providerGroupDto
    ){
        ProviderGroupDto newProviderGroup = providerGroupMapper.map(providerGroupService.create(providerGroupMapper.map(providerGroupDto)));
        return ResponseEntity.status(HttpStatus.CREATED).body(newProviderGroup);
    }
